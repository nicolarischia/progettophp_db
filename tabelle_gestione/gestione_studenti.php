...
if (isset($_POST['action']) && $_POST['action'] === 'delete') {
    ...
    $id = $data['id'];
    try {
        $stmt = $pdo->prepare("DELETE FROM studenti WHERE idStudente = ?");
        $stmt->execute([$id]);
...
if (isset($_POST['action']) && $_POST['action'] === 'edit') {
    $data = json_decode(file_get_contents('php://input'), true);
    try {
        $stmt = $pdo->prepare("UPDATE studenti SET 
            cognome = ?, 
            nome = ?, 
            dataDiNascita = ?, 
            email = ?, 
...
<head>
    ...
    <title>Gestione Studenti - PCTO</title>
...
<body>
    ...
    <div class="container">
                ...
                <tbody id="studentiTableBody">
                    <?php if (isset($studenti) && !empty($studenti)): ?>
                        <?php foreach ($studenti as $studente): ?>
                            <tr data-id="<?php echo htmlspecialchars($studente['idStudente']); ?>">
                                ...
                                <td>
                                    <button onclick="openEditModal(<?php echo htmlspecialchars(json_encode($studente)); ?>)" class="btn btn-edit">Modifica</button>
                                    <button onclick="openDeleteModal(<?php echo $studente['idStudente']; ?>, '<?php echo htmlspecialchars($studente['cognome'] . ' ' . $studente['nome']); ?>')" class="btn btn-delete">Elimina</button>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr>
                            <td colspan="7">Nessuno studente trovato</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
        ...
        <div class="button-container">
            <a href="../tabelle_inserisci/inserisci_studenti.php" class="btn btn-primary">Inserisci Nuovo Studente</a>
    ...
    <div id="editModal" class="modal">
        <div class="modal-content">
            ...
            <form id="editForm" onsubmit="return handleEditSubmit(event)">
                ...
                <div class="form-group">
                    <label for="edit_data">Data di Nascita:</label>
                    <input type="date" id="edit_data" name="dataDiNascita" required>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="edit_classe" name="classe" required>
                </div>
    ...
    <div id="deleteModal" class="modal">
        <div class="modal-content">
            ...
            <p>Sei sicuro di voler eliminare lo studente <span id="delete_name"></span>?</p>
    ...
    <script>
        ...
        function openEditModal(studente) {
            document.getElementById('edit_id').value = studente.idStudente;
            document.getElementById('edit_cognome').value = studente.cognome;
            document.getElementById('edit_nome').value = studente.nome;
            document.getElementById('edit_data').value = studente.dataDiNascita;
            document.getElementById('edit_email').value = studente.email;
            document.getElementById('edit_classe').value = studente.classe;
        ...
        async function handleEditSubmit(event) {
            ...
            try {
                ...
                if (result.success) {
                    showMessage('Studente modificato con successo!', 'success');
                    closeEditModal();
        ...
        async function handleDelete() {
            ...
            try {
                ...
                if (result.success) {
                    showMessage('Studente eliminato con successo!', 'success');
                    closeDeleteModal();
