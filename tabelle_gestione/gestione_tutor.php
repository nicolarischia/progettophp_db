...
if (isset($_POST['action']) && $_POST['action'] === 'delete') {
    ...
    $id = $data['id'];
    try {
        $stmt = $pdo->prepare("DELETE FROM tutoraziendali WHERE idTutor = ?");
        $stmt->execute([$id]);
...
if (isset($_POST['action']) && $_POST['action'] === 'edit') {
    $data = json_decode(file_get_contents('php://input'), true);
    try {
        $stmt = $pdo->prepare("UPDATE tutoraziendali SET 
            cognome = ?, 
            nome = ?, 
            email = ?, 
            telefono = ?, 
...
// Recupero dati
try {
    $stmt = $pdo->query("SELECT t.*, a.regioneSociale 
...
<head>
    ...
    <title>Gestione Tutor - PCTO</title>
...
<body>
    ...
    <div class="container">
                ...
                <tbody id="tutorTableBody">
                    <?php if (isset($tutor) && !empty($tutor)): ?>
                        <?php foreach ($tutor as $t): ?>
                            <tr data-id="<?php echo htmlspecialchars($t['idTutor']); ?>">
                                ...
                                <td><?php echo htmlspecialchars($t['regioneSociale']); ?></td>
                                <td>
                                    <button onclick="openEditModal(<?php echo htmlspecialchars(json_encode($t)); ?>)" class="btn btn-edit">Modifica</button>
                                    <button onclick="openDeleteModal(<?php echo $t['idTutor']; ?>, '<?php echo htmlspecialchars($t['cognome'] . ' ' . $t['nome']); ?>')" class="btn btn-delete">Elimina</button>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr>
                            <td colspan="7">Nessun tutor trovato</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
        ...
        <div class="button-container">
            <a href="../tabelle_inserisci/inserisci_tutor.php" class="btn btn-primary">Inserisci Nuovo Tutor</a>
                ...
                <div class="form-group">
                    ...
                    <select id="edit_idAzienda" name="idAzienda" required>
                        ...
                        while ($azienda = $stmt->fetch()) {
                            echo "<option value='" . $azienda['idAzienda'] . "'>" . htmlspecialchars($azienda['regioneSociale']) . "</option>";
                        }
                        ?>
                    </select>
                </div>
    ...
    <div id="deleteModal" class="modal">
        <div class="modal-content">
            ...
            <p>Sei sicuro di voler eliminare il tutor <span id="delete_name"></span>?</p>
    ...
    <script>
        ...
        function openEditModal(tutor) {
            document.getElementById('edit_id').value = tutor.idTutor;
            document.getElementById('edit_cognome').value = tutor.cognome;
            document.getElementById('edit_nome').value = tutor.nome;
            document.getElementById('edit_email').value = tutor.email;
            document.getElementById('edit_telefono').value = tutor.telefono;
            document.getElementById('edit_idAzienda').value = tutor.idAzienda;
        ...
        async function handleEditSubmit(event) {
            ...
            try {
                ...
                if (result.success) {
                    showMessage('Tutor modificato con successo!', 'success');
                    closeEditModal();
        ...
        async function handleDelete() {
            ...
            try {
                ...
                if (result.success) {
                    showMessage('Tutor eliminato con successo!', 'success');
                    closeDeleteModal();
